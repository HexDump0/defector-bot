// Population rollout candidate. Training snapshot: 2026-09-05_0905IST.
...
function rebuildMemory(history, previous) {
    ...
    for (let i = 0; i < history.length; i++) {
        ...
        if (round.you === "D" && round.opponent === "D") memory.mutualDStreak++;
...
function decide(history, memory) {
    ...
    if (pC >= COOPERATION_THRESHOLD) {
        const theirDWasProvoked =
            last.opponent === "D" && previous !== null && previous.you === "D";
...
function policy(id, h, start) {
        ...
        if (
            ...
            (h
                ...
                h
                    ...
                    .join("") === "CCCCD") &&
            !h.some(
                (r, i) => i && r.opponent === "D" && h[i - 1].opponent === "D",
            )
        )
            return "C";
    ...
    if (id === 4)
        return last?.opponent === "D" && prev?.opponent === "D" ? "D" : "C";
    if (id === 5)
        return last?.opponent === "D" && prev?.you !== "D" ? "D" : "C";
    if (id === 6)
        return h.slice(-3).every((r) => r.opponent === "D") && n >= 3
            ? "D"
            : "C";
    if (id >= 7 && id <= 16) {
        const patterns = [
        ...
        return patterns[id - 7][(n - start) % patterns[id - 7].length];
    }
    if (id >= 17 && id <= 22) {
    ...
    if (id >= 35 && id <= 40) {
        const periods = [
            ...
            "DDDDDDDDDDDDDDDDDDDDCCCCCCCCCCCCCCCCCCCC",
            "DCCCCCCCCCC",
            "DDCCCCCCCC",
        ];
        const seq = periods[id - 35];
        return seq[(n - start) % seq.length];
    }
    if (id >= 41 && id <= 45)
        return n - start < [5, 10, 20, 40, 60][id - 41] ? "C" : "D";
    if (id >= 46 && id <= 50)
        return n - start < [5, 10, 20, 40, 60][id - 46] ? "D" : "C";
    return "D";
}
const TREE = [
    [0, 1, 562, 0],
