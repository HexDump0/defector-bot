...
function decide(history, memory) {
	...
	// FARMER EXPERIMENT (v6 only, predicted NET NEGATIVE on today's field):
	// after 30 rounds of pristine mutual cooperation, defect forever. A pure
	// cooperator never retaliates (farmed at ~2.75); a grim punishes forever
	// (~1.25). Profitable only if farmable bots outnumber unforgiving ones.
	// Under opponentD == 0 any past our-D is exactly this probe, so no flag
	// needs storing; memory layout is unchanged from v5.
	if (memory.opponentD === 0 && n >= 30) return ["D", memory]

	// Never disturb pristine cooperation before the probe round.
